...
)
{
    ...
    for (int i = 1; i < comm->size; i++) {
        ...
        fgmp_srdy(fgmp_addr_from_rank(src, comm) + comm->root);
        if (src == dest) {            
            if (comm->rank < dest) {
                mpi_transfer_send(fgmp_addr_from_rank(dest, comm) + comm->root, sendcounts[dest] * sizeof_mpi_datatype(sendtype), (void*)sendbuf + sdispls[dest]);                
                mpi_transfer_recv(fgmp_addr_from_rank(src, comm) + comm->root, recvcounts[src] * sizeof_mpi_datatype(sendtype), recvbuf + rdispls[src]);
            } else {
                mpi_transfer_recv(fgmp_addr_from_rank(src, comm) + comm->root, recvcounts[src] * sizeof_mpi_datatype(sendtype), recvbuf + rdispls[src]);
                mpi_transfer_send(fgmp_addr_from_rank(dest, comm) + comm->root, sendcounts[dest] * sizeof_mpi_datatype(sendtype), (void*)sendbuf + sdispls[dest]);
            }
