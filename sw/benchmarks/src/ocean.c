// WCET-aware clone of OCEAN from the SPLASH-2 bechmark suite.
// Completely rewritten.
//
// The original implementation is available via the PARSEC website at
// http://parsec.cs.princeton.edu/parsec3-doc.htm#splash2
//
...
void forward_laplacian_edges(int m, int xm,  int ym)
...
void forward_edges_and_corners(int m, int xm,  int ym)
//                          memblock, lastcol, lastrow
{
...
void copy_rhs_borders(int k, int xm, int ym)
{
    int base = (ym+2) * sizeof(double*) / sizeof(double);
...
void forward_redblack(int memblock, unsigned rb, int xm, int ym)
{
    int base = (ym+2) * sizeof(double*) / sizeof(double);
...
double calc_residual(percore_data_t *percore, int k)
{
    ...
    forward_redblack(MB_Q+k, BLACK, lastcol, lastrow);
    ...
    forward_redblack(MB_Q+k, RED, lastcol, lastrow);
    residual = relax(percore->q[k], percore->rhs[k], 
...
void finer(percore_data_t *percore, int k)
{
    ...
    forward_edges_and_corners(MB_Q+k, lastcol, lastrow);
...
void coarser(percore_data_t *percore, int k)
{
    ...
    forward_edges_and_corners(MB_Q+k, lastcol, lastrow);
    copy_rhs_borders(k, lastcol, lastrow);
    rescal(percore, k);
...
void prepare(percore_data_t *percore)
{
    ...
    forward_laplacian_edges(MB_PSIB, lastcol, lastrow);
    ...
    for(i=ilo; i<=ihi; i++) {
	    ...
	    // compute input curl of wind stress
    	    percore->tauz[i][j] = const3 * harness_sin((double)(j+j_off)*const4);
...
void iteration(percore_data_t *percore)
{
    ...
    for(p=0; p<2; p++) {
        forward_laplacian_edges(MB_PSI+p, lastcol, lastrow);
    ...
    // W7_3 = laplacian(W5_3)
    for(p=0; p<2; p++) {
        forward_laplacian_edges(MB_W5+p, lastcol, lastrow);
    ...
    for (p=0; p<2; p++) {
        forward_edges_and_corners(MB_W1+p, lastcol, lastrow);
        forward_edges_and_corners(MB_PSI+p, lastcol, lastrow);
    ...
    // W4_3 = laplacian(W7_3)
    for(p=0; p<2; p++) {
        forward_laplacian_edges(MB_W7+p, lastcol, lastrow);
    ...
    // W6 = jacobian(W2, W3)
    forward_edges_and_corners(MB_W2, lastcol, lastrow);
    forward_edges_and_corners(MB_GA, lastcol, lastrow);
    ...
    // W2_3 = laplacian(W1_3)
    for(p=0; p<2; p++) {
        forward_laplacian_edges(MB_W1+p, lastcol, lastrow);
...
double **malloc_linkup(int handle, int x_part, int y_part)
{
    double **row_ptr = (double **)parop_def_memblock(handle,
...
void init(percore_data_t *percore)
{
    ...
    percore->lev_tol = parop_def_array_double(MB_LEVTOL, numlev);
    percore->lev_tol[numlev-1] = params->tolerance;

    percore->f = parop_def_array_double(MB_F, grid_width);
    ...
    percore->one_row = parop_def_array_double(MB_ROW, 
        1 << (numlev));
    percore->one_col = parop_def_array_double(MB_COL,
        1 << (numlev+(percore->params.log2_nprocs&1)));

    int x_part = (1 << (params->log2_grid - ((params->log2_nprocs)>>1))) + 2;
    int y_part = (1 << (numlev)) + 2;
    percore->ga    = malloc_linkup(MB_GA,   x_part, y_part);
    percore->gb    = malloc_linkup(MB_GB,   x_part, y_part);
    percore->psib  = malloc_linkup(MB_PSIB, x_part, y_part);
    percore->work2 = malloc_linkup(MB_W2,   x_part, y_part);
    percore->tauz  = malloc_linkup(MB_TAUZ, x_part, y_part);

    percore->psi[0]   = malloc_linkup(MB_PSI+0,   x_part, y_part);
    percore->psim[0]  = malloc_linkup(MB_PSIM+0,  x_part, y_part);
    percore->work1[0] = malloc_linkup(MB_W1+0,    x_part, y_part);
    percore->work5[0] = malloc_linkup(MB_W5+0,    x_part, y_part);
    percore->work7[0] = malloc_linkup(MB_W7+0,    x_part, y_part);
    percore->gamma[0] = malloc_linkup(MB_GAMMA+0, x_part, y_part);

    percore->psi[1]   = malloc_linkup(MB_PSI+1,   x_part, y_part);
    percore->psim[1]  = malloc_linkup(MB_PSIM+1,  x_part, y_part);
    percore->work1[1] = malloc_linkup(MB_W1+1,    x_part, y_part);
    percore->work5[1] = malloc_linkup(MB_W5+1,    x_part, y_part);
    percore->work7[1] = malloc_linkup(MB_W7+1,    x_part, y_part);
    percore->gamma[1] = malloc_linkup(MB_GAMMA+1, x_part, y_part);

    for (i=0; i<numlev; i++) {
        x_part = (1 << ((percore->params.log2_nprocs&1)+1+i))+2;
        y_part = (1 << (1+i))+2;
        percore->q  [i] = malloc_linkup(MB_Q  +i, x_part, y_part);
        percore->rhs[i] = malloc_linkup(MB_RHS+i, x_part, y_part);
    }
}
