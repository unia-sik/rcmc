...
PAROP_LOCAL uint64_t randNASseed = 314159265;
    ...
    double shift)
{
    ...
    bool mark[n+1];
#else
    double *aelt   = machine_malloc(sizeof(double)*(nz+1));
    double *v      = machine_malloc(sizeof(double)*(n+1+1));
    int32_t *arow  = machine_malloc(sizeof(int32_t)*(nz+1));
    int32_t *acol  = machine_malloc(sizeof(int32_t)*(nz+1));
    int32_t *nzloc = machine_malloc(sizeof(int32_t)*(n+1));
    bool    *mark  = machine_malloc(sizeof(bool)*(n+1));
#endif
...
void init(percore_data_t *percore, params_data_t *params)
{
    ...
    int exch_proc;
    if (pcols == prows) {
        exch_proc = (my_rank%prows) * prows + my_rank/prows;
    } else {
        machine_assert(pcols==2*prows && nrows==2*ncols);
        exch_proc = 2 * (((my_rank/2) % prows) * prows + my_rank/2/prows)
            + my_rank % 2;
    }
    parop_def_direction(D_TRANSPOSE, exch_proc, exch_proc);

    int i;
    int color[parop_max_rank];
    for (i=0; i<parop_max_rank; i++) color[i] = i/pcols;
    ...
    makea(n, nz, percore->a, percore->colidx, percore->rowstr, nonzer,
        firstrow, lastrow, firstcol, lastcol, 0.1, shift);
    ...
    percore->x = parop_def_array_double(A_X, nrows+2+1);
    percore->z = parop_def_array_double(A_Z, nrows+2+1);
    percore->p = parop_def_array_double(A_P, nrows+2+1);
    percore->q = parop_def_array_double(A_Q, nrows+2+1);
    percore->r = parop_def_array_double(A_R, nrows+2+1);
    percore->w = parop_def_array_double(A_W, nrows+2+1);
...
double conjugent_gradient(percore_data_t *percore, int n, int log2_num_procs)
{
    ...
    for (cgit=0; cgit<CG_ITERATIONS; cgit++) {
        ...
        parop_allreduce_multi_double(C_ROW, PAROP_OP_ADD, &q[1], nrows, &w[1]);
    ...
    double *rsquare, double *zeta)
{
    ...
    *zeta = params->eigenvalue_shift + 1.0 / total[0];
    ...
    double norm = 1.0 / harness_sqrt( total[1] );
    for (i=1; i<=nrows; i++)
